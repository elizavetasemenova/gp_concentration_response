---
title: "Changepoint Gaussian Process for concerntration-response curve fitting."
output: html_document
---

This tutorial provides commentary to the code supplied with the manuscript "Flexible fitting of PROTAC concentration-response curves with Gaussian Processes".
...
We read in raw data files and take a look a look at their content:
```{r}
...
To perform kriging, we need to define prediction points $x_\text{pred},$ i.e. the concentration at which we would like to obtain the estimates of the activity. We chose a uniform grid between the minimal and the maximal measured conccentrations:
```{r}  
  N_pred <- 200
